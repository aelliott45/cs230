...
def train():
    ...
    # Build loss (can be single-task or MultiTaskIRDropLoss)
    ...
    print("MAX ITERS:", arg_dict['max_iters'], " Type:", type(arg_dict['max_iters']))
    ...
    while iter_num < arg_dict['max_iters']:
        with tqdm(total=print_freq) as bar:
            for feature, label, _ in dataset:
                ...
                # label can be:
                #   • tensor              (single-task)
                #   • (reg_t, cls_t) tuple (multitask)
                # -----------------------------
                if arg_dict['cpu']:
                    inputs = feature
                    targets = label
                else:
                    inputs = feature.cuda(non_blocking=True)

                    if isinstance(label, (tuple, list)):
                        # multitask: e.g. (reg_target, cls_target)
                        targets = tuple(
                            t.cuda(non_blocking=True) for t in label
                        )
                    else:
                        # single-task: tensor
                ...
                optimizer.zero_grad()
                pixel_loss = loss(prediction, targets)
                ...
                if iter_num % print_freq == 0 or iter_num >= arg_dict['max_iters']:
                    break
